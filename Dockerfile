...
ARG REPO_URL=https://github.com/ShayNeeo/W9.git
ARG GIT_REF=main

WORKDIR /app

# Pull lightweight commit metadata so the cache invalidates whenever the main branch moves.
ADD https://api.github.com/repos/ShayNeeo/W9/git/refs/heads/main /tmp/git-ref.json
...
RUN cargo build --release --package w9
...
RUN useradd -m -u 1000 w9user

WORKDIR /app

# Copy binaries and assets from builder
COPY --from=builder /app/target/release/w9 /app/w9
COPY --from=builder /app/static /app/pkg

# Create uploads directory with proper permissions
RUN mkdir -p /app/uploads && \
    chown -R w9user:w9user /app
